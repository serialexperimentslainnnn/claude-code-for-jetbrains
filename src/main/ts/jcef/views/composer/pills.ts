(function () {
  ...
  function providerMarkSvg(id: string): string {
    const inner = PROVIDER_MARKS[id] || PROVIDER_MARKS.anthropic;
    return '<svg viewBox="0 0 16 16" aria-hidden="true">' + inner + '</svg>';
  }
  function chipIconSvg(key: string): string | null {
    if (key === 'provider') return providerMarkSvg('anthropic');
    const inner = CHIP_ICONS[key];
  ...
  CX.buildPill = function (def: PillDef): Pill {
    const label = h('span', { class: 'pill-label', text: '' });
    const caret = h('span', { class: 'pill-caret', text: '▾' });
    const iconMarkup = chipIconSvg(def.key);
    const icon = iconMarkup ? h('span', { class: 'pill-icon', html: iconMarkup }) : null;
    const el = h(
      'button',
      {
        class: 'pill',
  ...
  function pillLabelFor(s: ComposerState, def: PillDef): string | null {
    const f = s[def.field] as PillField | undefined;
    if (!f) return null;
    if (f.label != null) return String(f.label);
    return null;
  }

  CX.renderPills = function (s: ComposerState): void {
    const els = CX.els;
    if (!els || !els.pills) return;
    for (let i = 0; i < PILL_DEFS.length; i++) {
      const def = PILL_DEFS[i];
      const pill = els.pills[def.key];
      if (!pill) continue;
      const label = pillLabelFor(s, def);
      const f = s[def.field] as PillField | undefined;
      const hasOpts = f && Array.isArray(f.options) && f.options.length > 0;
      if (label == null) {
  ...
  CX.syncOpenMenu = function (): void {
    const openMenu = CX.openMenu;
    if (openMenu && openMenu.pill !== '__attach') {
      const key = openMenu.pill;
      let def: PillDef | null = null;
      for (let i = 0; i < PILL_DEFS.length; i++) if (PILL_DEFS[i].key === key) def = PILL_DEFS[i];
