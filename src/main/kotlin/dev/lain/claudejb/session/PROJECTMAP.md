...
|---|---|---|---|
| `AgentEnding` | object | `AgentEnding.kt:28` | What an agent's own transcript says about whether it is over — the only evidence there is about an agent the plugin … |
| `AgentEnding.of` | fun | `AgentEnding.kt:48` | `null` when there is nothing to judge (no transcript yet, or nothing parseable). |
...
| `WorkloadWindow.label` | fun | `WorkloadWindow.kt:41` | How a window reads in a menu: `15 Minutes`, `2 Hours`, `All in this session`. |
| `WorkloadWindow.RUN_STARTED_AT` | val | `WorkloadWindow.kt:59` | When this run of the plugin began watching workloads — one instant, captured once, shared by everything. |
| `WorkloadWindow.isVisible` | fun | `WorkloadWindow.kt:74` | Whether a workload belongs in the view under the given window. |
| `WorkloadWindow.visible` | fun | `WorkloadWindow.kt:121` | Which workloads belong in the view, judged BOTTOM-UP so that what is emitted is a tree that holds together. |
