...
class GitContextActionsTest {
    ...
    fun `the ui git entries reach nothing but the read-only service and navigator`() {
        ...
        val gitImports = code.map { it.trim() }
            .filter { GIT_IMPORT_PREFIXES.any(it::startsWith) }
        assertEquals(ALLOWED_GIT_IMPORTS, gitImports.toSet(), "Unexpected Git import(s) in the UI entry point: $gitImports")
        val siblings = code.flatMap { line -> GIT_SIBLINGS.filter { it in line } }.toSet()
        assertEquals(GIT_SIBLINGS, siblings, "The Git menu no longer reaches the read-only service and navigator: $siblings")
    ...
    fun `the branch-titled chooser reads one branch, and only the graph reads every line`() {
        val menu = File("src/main/kotlin/dev/lain/claudejb/controller/commands/git/GitContextActions.kt").readLines()
            .filterNot { line ->
                val trimmed = line.trimStart()
                trimmed.startsWith("*") || trimmed.startsWith("//") || trimmed.startsWith("/*")
            }
        ...
        val dashboard = File("src/main/kotlin/dev/lain/claudejb/controller/commands/git/GitIntegration.kt").readText()
        assertTrue(
        ...
        val service = File("src/main/kotlin/dev/lain/claudejb/controller/git/GitHistoryService.kt").readText()
        assertTrue(
    ...
    fun `the entries hide themselves when git is unavailable, they do not grey out`() {
        val source = File("src/main/kotlin/dev/lain/claudejb/controller/commands/git/GitContextActions.kt").readLines()
            .map { it.trim() }
    ...
    private companion object {
        ...
        val GIT_IMPORT_PREFIXES = listOf(
            "import dev.lain.claudejb.model.git.",
            "import dev.lain.claudejb.controller.git.",
            "import git4idea",
        )

        val GIT_SIBLINGS = setOf("GitHistoryService", "GitLogNavigator")
