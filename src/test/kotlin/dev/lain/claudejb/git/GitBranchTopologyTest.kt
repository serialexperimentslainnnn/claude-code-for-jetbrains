...
/**
 * The pure half of the branch topology: the model's absences, and reading the number `git rev-list --count`
 * printed.
 *
 * Nothing here spawns `git`. The two questions worth asking without a repository are whether *unknown* survives as
 * a distinct answer from *zero* — they mean opposite things to a reader, "we could not tell" against "you are in
 * sync" — and whether the empty answer is genuinely empty, since that is what every degradation path returns.
 */
class GitBranchTopologyTest {
    ...
    fun `null in is null out, because that is how the platform reports a failed command`() {
        // It catches the VcsException, logs it at debug and returns null. Null is the ONLY signal a caller gets.
        assertNull(GitBranchTopology.commitCount(null))
    }

    @Test
    fun `output that is not a number is unknown, never zero`() {
        // Zero is the most reassuring thing this model can say — "in sync". Inferring it from output nobody could
        // parse is how a branch that is eight commits behind reports itself as up to date.
    ...
    fun `a branch that tracks nothing carries the branch and nothing else`() {
        // The shape the gateway returns for a local-only branch: there is no reference point, so a count would be
        // an answer to a question nobody can ask — and 0/0 would read as "in sync with an upstream".
    ...
    fun `two topologies with the same fields are the same value`() {
        // Same reason as GitCommitInfo: the views re-push their model on every tick and skip the repaint when
        // nothing changed. Equality on the model is what that skip is made of.
