...
class MarkdownRendererTest {
    ...
    @Test
    fun `path-line inside backticks becomes a jb open link wrapped in code`() {
        // Code spans were being masked before the bare-text linkifier, so backticked path:line never linkified.
        // Now the code-span restoration detects the pattern and emits <code><a href="jb://open…">…</a></code>.
        val out = html("see `src/main/kotlin/dev/lain/claudejb/session/SessionStore.kt:35` for the guard")
        assertTrue(out.contains("<code><a href=\"jb://open?file="), out)
        assertTrue(out.contains("&amp;line=35\">"), out)
        assertTrue(out.contains("SessionStore.kt:35</a></code>"), out)
    }

    @Test
    fun `non-path content inside backticks stays as plain code`() {
        val out = html("call `foo()` to start")
        assertTrue(out.contains("<code>foo()</code>"), out)
        assertFalse(out.contains("jb://open"), out)
    }
