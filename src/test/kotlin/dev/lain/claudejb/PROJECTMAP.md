...
| `GitApiContractTest` | class | `git/GitApiContractTest.kt:33` | Pins **every Git4Idea / VCS API this package calls** against the platform we build on, and pins that none of them is … |
...
| `GitReadOnlyContractTest` | class | `git/GitReadOnlyContractTest.kt:34` | **The Git integration is read-only, and this is what makes that true rather than merely written down.** The decision … |
...
| `UsageReportTest` | class | `protocol/UsageReportTest.kt:26` | [parseUsageReport] against the shape the binary really returns. |
...
| `GitActionCatalogTest` | class | `ui/GitActionCatalogTest.kt:22` | The Git action catalogue, pinned where it is a contract with something that cannot check it back. |
...
| `GitPromptedActionsTest` | class | `ui/GitPromptedActionsTest.kt:20` | The prompts the Git integration sends, pinned. |
