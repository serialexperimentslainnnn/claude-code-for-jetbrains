...
class PrivilegeEscalationFuzzTest : GuardProbe(GuardFixture.basePolicy().copy(guardedRoots = emptyList(), wslHost = false)) {
    ...
    fun `padding an escalator with whitespace does not hide it`() {
        ...
        repeat(300) {
            ...
            assertEquals(Verdict.DENY, v(payload("command", command)), command)
        }
    }
    ...
    fun `su is matched as a command and never inside a longer word`() {
        listOf("su ls", "su - root", "echo hi; su", "/bin/su -").forEach {
            assertEquals(Verdict.DENY, v(payload("command", it)), it)
        }
        listOf("npm run superbuild", "git submodule update", "echo summary", "ls subdir").forEach {
            assertEquals(Verdict.ALLOW, v(payload("command", it)), it)
        }
    }
    ...
    fun `an escalator named but not at a command position is never a hit`() {
        ...
        repeat(300) {
            ...
            assertEquals(Verdict.ALLOW, v(payload("command", command)), command)
        }
    }
    ...
    fun `no escalator reaches the rules through a payload that is not a command`() {
        ...
        repeat(300) {
            ...
            assertEquals(Verdict.ALLOW, v(input), input.toString())
        }
    }
}
