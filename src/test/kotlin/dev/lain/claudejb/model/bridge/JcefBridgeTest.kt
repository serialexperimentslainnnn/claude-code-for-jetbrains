...
class JcefBridgeTest {
    ...
    fun `parse send carries text`() {
        ...
        assertEquals("hello world", (m as Msg.Send).text)
    }
    ...
    fun `parse change messages`() {
        assertEquals(
            "claude-opus-4-8",
            (JcefBridge.parse("""{"type":"changeModel","value":"claude-opus-4-8"}""") as Msg.ChangeModel).value,
        )
        assertNull((JcefBridge.parse("""{"type":"changeModel"}""") as Msg.ChangeModel).value)
        assertEquals("plan", (JcefBridge.parse("""{"type":"changeMode","wire":"plan"}""") as Msg.ChangeMode).wire)
        assertNull((JcefBridge.parse("""{"type":"changeEffort","value":null}""") as Msg.ChangeEffort).value)
        assertEquals("high", (JcefBridge.parse("""{"type":"changeEffort","value":"high"}""") as Msg.ChangeEffort).value)
        assertTrue((JcefBridge.parse("""{"type":"changeThinking","on":true}""") as Msg.ChangeThinking).on)
        assertEquals("deepseek", (JcefBridge.parse("""{"type":"changeProvider","id":"deepseek"}""") as Msg.ChangeProvider).id)
        assertEquals(2, (JcefBridge.parse("""{"type":"removeQueued","index":2}""") as Msg.RemoveQueued).index)
    }

    @Test
    fun `parse permission resolutions`() {
        val rp = JcefBridge.parse("""{"type":"resolvePermission","id":"r9","allow":true}""") as Msg.ResolvePermission
        assertEquals("r9", rp.id)
        assertTrue(rp.allow)
        val rq = JcefBridge.parse(
            """{"type":"resolveQuestion","id":"r9","answers":{"Q1":"A","Q2":"B"}}""",
        ) as Msg.ResolveQuestion
        assertEquals(mapOf("Q1" to "A", "Q2" to "B"), rq.answers)
        assertEquals("Edit", (JcefBridge.parse("""{"type":"alwaysAllow","tool":"Edit"}""") as Msg.AlwaysAllow).tool)
        assertEquals("r9", (JcefBridge.parse("""{"type":"viewDiff","id":"r9"}""") as Msg.ViewDiff).id)
    }

    @Test
    fun `parse open and copy`() {
        assertEquals("https://x.dev", (JcefBridge.parse("""{"type":"open","url":"https://x.dev"}""") as Msg.Open).url)
        assertEquals("snippet", (JcefBridge.parse("""{"type":"copy","text":"snippet"}""") as Msg.Copy).text)
    }
    ...
    fun `parse removeAttachment carries id`() {
        val m = JcefBridge.parse("""{"type":"removeAttachment","id":"att7"}""") as Msg.RemoveAttachment
    ...
    fun `an absent path is the project ROOT, not a missing field`() {
        val m = JcefBridge.parse("""{"type":"treeChildren","mode":"files"}""") as Msg.TreeChildren
        assertEquals("", m.path)
    }
    ...
    fun `a path that is not a string is dropped, never thrown on`() {
        ...
        assertEquals("", (obj as Msg.TreeChildren).path)
        val arr = JcefBridge.parse("""{"type":"treeExpand","path":["a"],"mode":"files"}""")
        assertEquals("", (arr as Msg.TreeExpand).path)
    }
    ...
    fun `an absolute path rides through verbatim — containment is ProjectTree's single gate`() {
        ...
        assertEquals(listOf("../../etc/passwd"), (up as Msg.AttachPaths).paths)
    }
    ...
    fun `parse attachPaths is a batch, and total on the shapes a batch can arrive in`() {
        ...
        assertTrue((JcefBridge.parse("""{"type":"attachPaths"}""") as Msg.AttachPaths).paths.isEmpty())
    }

    @Test
    fun `parse mcpReconnect carries name`() {
        val m = JcefBridge.parse("""{"type":"mcpReconnect","name":"jetbrains"}""") as Msg.McpReconnect
    ...
    fun `parse mcpToggle carries name and enabled flag`() {
        ...
        val off = JcefBridge.parse("""{"type":"mcpToggle","name":"srv","enabled":false}""") as Msg.McpToggle
    ...
    fun `parse stopTask carries taskId`() {
        val m = JcefBridge.parse("""{"type":"stopTask","taskId":"task42"}""") as Msg.StopTask
    ...
    fun `parse gitAction without a hash reads as no commit, not as a missing field`() {
        val m = JcefBridge.parse("""{"type":"gitAction","id":"commit"}""") as Msg.GitAction
    ...
    fun `a turn carries the conversation it is for, and the ordinary one carries none`() {
        assertEquals(Msg.Send("hello"), JcefBridge.parse("""{"type":"send","text":"hello"}"""))
        assertEquals(
            Msg.Send("squash those two", JcefBridge.SCOPE_GIT),
            JcefBridge.parse("""{"type":"send","text":"squash those two","scope":"git"}"""),
        )
        assertEquals(Msg.Interrupt(), JcefBridge.parse("""{"type":"interrupt"}"""))
        assertEquals(
            Msg.Interrupt(JcefBridge.SCOPE_GIT),
    ...
    fun `parse a card resolution carries the conversation it belongs to`() {
        ...
        assertEquals(Msg.ResolvePermission("r1", true, ""), own)
        val git = JcefBridge.parse("""{"type":"resolvePermission","id":"r1","allow":true,"scope":"git"}""")
        assertEquals(Msg.ResolvePermission("r1", true, JcefBridge.SCOPE_GIT), git)
    }

    @Test
    fun `parse installClaude carries the method id`() {
        val m = JcefBridge.parse("""{"type":"installClaude","method":"apt"}""") as Msg.InstallClaude
        assertEquals("apt", m.method)
    }

    @Test
    fun `parse setBinaryPath carries the path`() {
        val m = JcefBridge.parse("""{"type":"setBinaryPath","path":"/opt/claude/claude"}""") as Msg.SetBinaryPath
        assertEquals("/opt/claude/claude", m.path)
    }

    @Test
    fun `parse recheckBinary`() {
        assertEquals(Msg.RecheckBinary, JcefBridge.parse("""{"type":"recheckBinary"}"""))
    }

    @Test
    fun `parse loginSubscription cancelLogin dismissAuth logout`() {
        assertEquals(Msg.LoginSubscription, JcefBridge.parse("""{"type":"loginSubscription"}"""))
        assertEquals(Msg.CancelLogin, JcefBridge.parse("""{"type":"cancelLogin"}"""))
        assertEquals(Msg.DismissAuth, JcefBridge.parse("""{"type":"dismissAuth"}"""))
        assertEquals(Msg.Logout, JcefBridge.parse("""{"type":"logout"}"""))
    }

    @Test
    fun `parse useApiKey and submitLoginCode carry their secret verbatim`() {
        val key = JcefBridge.parse("""{"type":"useApiKey","key":"sk-ant-test-123"}""") as Msg.UseApiKey
        assertEquals("sk-ant-test-123", key.key)
        val code = JcefBridge.parse("""{"type":"submitLoginCode","code":"ABC-42"}""") as Msg.SubmitLoginCode
    ...
    fun `parse resolveLinks is total on missing, blank and non-string candidates`() {
        val m = JcefBridge.parse("""{"type":"resolveLinks"}""") as Msg.ResolveLinks
    ...
    fun `parse unknown type and malformed are total`() {
        assertTrue(JcefBridge.parse("""{"type":"wat"}""") is Msg.Unknown)
        assertTrue(JcefBridge.parse("not json") is Msg.Unknown)
        assertTrue(JcefBridge.parse("""{"notype":1}""") is Msg.Unknown)
    }
}
