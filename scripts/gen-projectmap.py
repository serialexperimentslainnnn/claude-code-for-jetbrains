...
`--check` regenerates in memory and diffs against disk, and it is a CONVENIENCE, not a gate. It used to be
wired into `./gradlew check` and into CI, and that was wrong twice over: these maps are an orientation index
for AI-assisted sessions, excluded from the artifact, so a stale one cannot reach anybody who installs the
plugin — it made the only build failure that is never a defect in the product, and it put a Python script
between a contributor and a green build for editing a file. A missing map is an ordinary divergence: every
target is reported, so a run says which packages have no map rather than stopping at the first one that does
not.
...
Run it with no arguments to rewrite every generated block, or with `--check` to diff against disk instead
and fail on any divergence.

It does not quote its own command line anywhere, and that is deliberate rather than cosmetic: a script that
contains `<interpreter> <its own path>` is, to anything reading scripts statically, a script that runs
itself. Say which flags exist, not how to type the command.
"""
...
SELF_PATH = "scripts/gen-projectmap.py"
...
# A package earns a local map once it is too big to read whole; below that, an index is more upkeep than
# help. Five is where this repository's own packages divide, and it is why `actions` and `util` carry none.
MIN_INDEXABLE_FILES = 5

# Where the plugin's own packages live. Everything under it is DISCOVERED rather than listed, so a package
# added tomorrow gets its map without anybody remembering to come here.
PACKAGE_ROOT = "src/main/kotlin/dev/lain/claudejb"

# The trees that get ONE map for the whole tree instead of one per directory, because that is how they are
# read: a web app, a test pyramid, a suite, a shelf of documents. These have to be named — no property of
# their contents distinguishes them from any other folder of files.
ANCHORS = [
...
def block_of(target: Path, kind: str, nested: list[Path]) -> str:
    body = [
        f"<!-- Generated by {SELF_PATH}. Everything between these markers is overwritten on the",
...
def holds_enough_kotlin(directory: Path) -> bool:
    """Whether this directory's OWN files (not its children's) are numerous enough to want an index."""
    own = [path for path in directory.iterdir() if path.is_file() and path.suffix == ".kt"]
    return len(own) >= MIN_INDEXABLE_FILES


def discovered_packages(root: Path) -> list[Path]:
    """Every package under `root` that earns a map, found by looking rather than by being told."""
    return [path for path in sorted(root.rglob("*")) if path.is_dir() and holds_enough_kotlin(path)]


def targets() -> list[tuple[Path, str]]:
    """Every directory that gets a map, with the kind of index it wants."""
    found = [(path, KOTLIN) for path in discovered_packages(ROOT / PACKAGE_ROOT)]
    for path, kind in ANCHORS:
        anchor = ROOT / path
        if not anchor.is_dir():
            raise SystemExit(f"gen-projectmap: {anchor} is an anchor but does not exist")
        found.append((anchor, kind))
    return found
...
def main() -> int:
    ...
    stale = []
    for target, kind in mapped:
        nested = [other for other, _ in mapped if other != target and other.is_relative_to(target)]
    ...
    print(
        ...
        f"— re-run {SELF_PATH} with no arguments",
        file=sys.stderr,
    )
    return 1
