...
- **Sign-in lives in a card, not a command — and the credential does not live on your disk.** Signed out, the card is the first thing a tab shows, before a turn can fail on it. The subscription flow is fully native and requests the **full OAuth consent** — the reduced `setup-token` grant drops scopes Claude Code exercises, file upload among them, which is what a pasted attachment travels on: the binary's browser flow runs under a hidden PTY, the card shows the URL (copyable) and completes on its own when the browser finishes, so pasting the code is an optional fallback on the same screen rather than a step of its own.
  ...
  For the session itself the credential is handed to the binary through a private configuration directory on **tmpfs** — memory, not a disk, wiped at reboot — owner-only, with the credential `0600`. Your real configuration (session history, settings, skills, account profile) is symlinked in, so `/resume` and everything else behave exactly as before, and the dashboard gets the full account and plan-limit picture. Token refresh happens there and the refreshed value returns to the safe when the session ends, so a login stays alive. Where there is no tmpfs, an environment token is used instead — never a credential written quietly to a real disk.
